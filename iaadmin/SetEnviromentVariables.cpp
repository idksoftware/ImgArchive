...
FolderOptions SetEnviromentVariables::processFolders(std::string& optionString)
{
	...
	else if (iequals(optionString, MASTER_BACKUP_ONE_LABEL)) {
		return FolderOptions::FO_MASTER_BACKUP_ONE;
	}
	else if (iequals(optionString, MASTER_BACKUP_TWO_LABEL)) {
		return FolderOptions::FO_MASTER_BACKUP_TWO;
	}
	else if (iequals(optionString, MASTER_PATH_LABEL)) {
		return FolderOptions::FO_DERIVATIVE_PATH;
	}
	else if (iequals(optionString, DERIVATIVE_BACKUP_ONE_LABEL)) {
		return FolderOptions::FO_DERIVATIVE_BACKUP_ONE;
	}
	else if (iequals(optionString, DERIVATIVE_BACKUP_TWO_LABEL)) {
...
bool SetEnviromentVariables::parseEnableOptions(const char* optionString, const char* value)
{
	...
	else if (iequals(optionString, WORKSPACE_LABEL)) {
		m_option = IMGA_WORKSPACE_ENABLE;
		return true;
	}
	else if (iequals(optionString, WORKSPACE_CHECKOUT_LABEL)) {
