...
namespace simplearchive {
	...
	/*
	bool DerivativeRepository::Import(ImageGroup &imageGroup) {
	...
	*/
	...
	bool DerivativeRepository::checkout(const char *filepath, const char *comment) {
	...
	bool DerivativeRepository::checkin(const char *filepath, const char *comment) {
	...
	bool DerivativeRepository::uncheckout(const char *filepath, const char *comment) {
		std::string fullPath = m_pathToArchive + "/" + filepath;
		std::string filename;
		std::string path;
		std::string buff;
	...
	bool ArchiveRepository::processHistory(ImagePath &imagePath, const char *filepath, const char *comment, const HistoryEvent &he, int ver) {
		...
		//printf("Path to Image in achive invalid \"%s\"", filepath);
		//}
		std::string hstpath = imagePath.getMasterHistoryPath();
		/*
	...
	DerivativeRepository &DerivativeRepository::get() {
		if (m_This == 0) {
			m_This = new DerivativeRepository();
		}
		return *m_This;
	}

	DerivativeRepository *DerivativeRepository::m_This = 0;
